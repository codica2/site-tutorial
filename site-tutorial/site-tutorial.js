...
function () {
  function SiteTutorial() {
    ...
    if (document.readyState == "interactive") {
      ...
      this.padding = this.config.padding > 15 ? 10 : Math.max(0, this.config.padding);
      this.offset = 10;
  ...
  _createClass(SiteTutorial, [{
    ...
    value: function startAnimation(commonStep, stepTo) {
      ...
      var draw = function draw(div) {
        ...
        canvas.height = _this4.getCommonHeightdocument();
        ctx.clearRect(0, 0, canvas.width, canvas.height); // letf rectangle

        ctx.fillStyle = "rgba(0,0,0, ".concat(config.opacity || 0.7, ")");
        ctx.fillRect(0, 0, Math.max(0, div.divX - padding), Math.max(0, canvas.height)); // top rectangle

        ctx.fillStyle = "rgba(0,0,0, ".concat(config.opacity || 0.7, ")");
        ctx.fillRect(Math.max(0, div.divX - padding < 0 ? 0 : div.divX - padding), 0, Math.max(0, canvas.width), Math.max(0, div.divY - padding)); // right rectangle

        ctx.fillStyle = "rgba(0,0,0, ".concat(config.opacity || 0.7, ")");
        ctx.fillRect(Math.max(0, div.divX + div.divWidth + padding), Math.max(0, div.divY - padding), Math.max(0, canvas.width - div.divX - padding), Math.max(0, canvas.height)); // bottom rectangle

        ctx.fillStyle = "rgba(0,0,0, ".concat(config.opacity || 0.7, ")");
        ctx.fillRect(Math.max(0, div.divX - padding), Math.max(0, div.divY + div.divHeight + padding), Math.max(0, div.divWidth + padding * 2), Math.max(0, canvas.height)); // light area
